		...
		for y in range(world_y_size):
			...
			if ht.map[x][y]<w.SeaLevel&& tm.map[x][y]>((w.IcebergThershold-0.01)*(w.TemperatureHigh-w.TemperatureLow)+w.TemperatureLow):
				...
				if ht.map[x][y]>=w.MountainThreshold && vg.map[x][y]>=w.ForestThreshold:
					...
					continue
				if ht.map[x][y]>=w.MountainThreshold && vg.map[x][y]<w.ForestThreshold && vg.map[x][y]>=0.125:
					map[x][y]=biome_codes["MountainGrassland"]
					continue
				if ht.map[x][y]>=w.MountainThreshold && vg.map[x][y]<0.125:
					map[x][y]=biome_codes["BarrenMountain"]
					continue
				if vg.map[x][y]<0.125:
					map[x][y]=biome_codes["Desert"]
					continue
				if vg.map[x][y]>=0.125 && vg.map[x][y]<w.ForestThreshold && dr.map[x][y]>=rf.map[x][y]-0.1375 && ht.map[x][y]<w.MountainThreshold:
					map[x][y]=biome_codes["Grassland"]
					continue
				if dr.map[x][y]<rf.map[x][y]-0.1375 && vg.map[x][y]<w.ForestThreshold && ht.map[x][y]<w.MountainThreshold:
					map[x][y]=biome_codes["Marshland"]
					continue
				if dr.map[x][y]<rf.map[x][y]-0.1375 && vg.map[x][y]>=w.ForestThreshold && ht.map[x][y]<w.MountainThreshold:
					map[x][y]=biome_codes["Swamp"]
					continue
				if dr.map[x][y]>=rf.map[x][y]-0.1375 && vg.map[x][y]>=w.ForestThreshold && ht.map[x][y]<w.MountainThreshold && tm.map[x][y]<((1-w.IcebergThershold+0.01)*(w.TemperatureHigh-w.TemperatureLow)+w.TemperatureLow) && tm.map[x][y]>((w.IcebergThershold-0.01)*(w.TemperatureHigh-w.TemperatureLow)+w.TemperatureLow) :
