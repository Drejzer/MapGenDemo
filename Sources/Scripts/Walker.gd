...
func _process(delta: float):
	if Input.is_key_pressed(KEY_LEFT):
		self.position.x=abs(int(barrier_x+1+self.position.x-speed*speedmod*delta*10.0)%(barrier_x+1))
	if Input.is_key_pressed(KEY_RIGHT):
		self.position.x=abs(int(self.position.x+speed*speedmod*delta*10.0)%(barrier_x+1))
	if Input.is_key_pressed(KEY_UP):
		if self.position.y>0:
			self.position.y-=speed*speedmod*delta*10.0
