...
func Generate(args):
	...
	for x in range(world_x_size):
		...
		for y in range(world_y_size):
			...
			noiseval=(0.5+(0.4*(tm.map[x][y]-par.TemperatureLow)/(par.TemperatureHigh-par.TemperatureLow)))*noiseval+0.875*(rfm.map[x][y]-drm.map[x][y])
			
			if hm.map[x][y]>=par.MountainThreshold/2.0+0.5 or hm.map[x][y]<=par.SeaLevel-0.0125 or (tm.map[x][y]-par.TemperatureLow)/(par.TemperatureHigh-par.TemperatureLow)<=par.IcebergThershold*0.75 or (tm.map[x][y]>=par.TemperatureLow*0.15+par.TemperatureHigh*0.85 && rfm.map[x][y]<=0.25):
				noiseval*=0.1
				...
				for i in range(3):
						for j in range(3):
							if y-1+j<0 || y-1+j>=world_y_size:
								...
								rivcount+=1
				noiseval=clamp(noiseval+0.125*rivcount,0.0,2.0)/2.0
